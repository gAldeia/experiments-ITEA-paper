...
//MODIFICAÇÃO: Criação da constante PI para uso em funções trigonométricas
// (as funções recebem a entrada em radiano, aqui eu faço essa conversão)
...
//PONTO DE INTERESSE: Esse valor determina o número de símbolos funcionais que existem
// e define o tamanho de um array que guarda esses símbolos. Ao criar novos símbolos, 
// devemos lembrar que existe uma variável global que faz essa contagem (e define o tamanho
// do array de símbolos, portanto devemos tomar cuidado com segmentation fault)
...
//PONTO DE INTERESSE: Essa é a classe que representa funções nos nós das árvores.
// Devemos consultar os parâmetros para saber o que atribuir na criação de uma nova função
class symbol{
	public:
...
//PONTO DE INTERESSE: Essa função faz a leitura do arquivo com os indivíduos e trace
// da última execução, para obter a melhor expressão de uma outra execução. Isso só é utilizado se
// na configuração a opção USE_TEST_SET estiver configurada. NOTE QUE se for feita a criação de novos
// símbolos, devemos tratar eles aqui no parse, para que a leitura e o parse possa ser feito sem gerar erros.
//OBSERVAÇÃO: APARENTEMENTE HÁ UM BUG NA VERSÃO ORIGINAL, pois ao utilizar a opção num_random_constants na execução com 
// USE_TEST_SET = 0, se utilizarmos o parser (quando USE_TEST_SET = 1) depois para avaliar uma expressão
// obtida em outra execução, ele gera erro e o programa dá crash.
node* parse_expression( char expression[]){
	...
	while(i<strlen(expression)){
		...
		if(expression[i]==' ')
			...
			while(!end_symbol){
				...
				//MODIFICAÇÃO: Como criei novos símbolos, coloquei como prefixo em todos eles o caractere '_'. Modifiquei esse trecho do parser
				// para reconhecer que quando ver um '_' ele está pegando um símbolo criado por mim.
				if(expression[i]=='(' || expression[i]==')' || expression[i]=='-' || expression[i]=='+' || expression[i]=='*' || expression[i]=='/' || expression[i]=='_'){ 
					...
					//MODIFICAÇÃO: ao contrário dos símbolos das funções originais, os que eu criei são
					// representados por uma sctring com mais de 1 caractere. Aqui eu reconheço qual das funções
					// está sendo utilizada, e uso o índice que eu atribui para elas na hora da criação (mais pra frente)
					// para criar o nó correspondente no parser
					if(expression[i]=='_'){ 

						if(expression[i+1]=='s' && expression[i+2]=='q'){//raiz quadrada
							el->root=symbols[6];

							// Atualizar o i de acordo com o tamanho desse texto
							i += 8;
						}
						if(expression[i+1]=='s' && expression[i+2]=='i'){//seno
							el->root=symbols[7];

							i += 3;
						}
						if(expression[i+1]=='c' && expression[i+2]=='o'){//cosseno
							el->root=symbols[8];
...
//OBSERVAÇÃO: Aqui ele lê do arquivo de configuração a opção mutation_step, mas na prática essa configuração nunca é
// utilizada!!! Talvez seja um bug, talvez o implementador percebeu depois que usar de forma aleatória é melhor e esqueceu
// de mudar, ou talvez ... ?
void read_config_file(cfg *config){
...
void create_T_F(){
	...
	//MODIFICAÇÃO: número de símbolos funcionais, sempre que eu criar um novo eu preciso aumentar isso (original =4).
	// Esse valor determina o tamanho de um array, usar um número menor gera segmentation fault, usar um número maior 
	// gera vetor com lixo dentro. Todas as funções devem ser contadas, mas note que há uma diferença
	// entre essa variável e a próxima (que considera só os parênteses)
    NUM_FUNCTIONAL_SYMBOLS=10; 

	NUM_AUXILIARY_FUNCTIONAL_SYMBOLS=2;

	//OBSERVAÇÃO: Essas são as funções originais
	...
	//MODIFICAÇÃO: preciso criar o símbolo na árvore. Construtor: symbol(bool p_type, int p_arity, int p_id, const char *p_name, int p_precedence)	
	// Lembrar de ver a aridade, e incrementar o ID. A precedência é utilizada apenas para transformar eq. matemática em árvore.
	// a primeira variável, o tipo, deve ser sempre 1 pois é um símbolo de função (e não terminal ou variável).
	// Para criar um símbolo, definimos esses valores e colocamos em symbols. NOTE QUE quem faz a computação é a função eval, que lê o nome da
	// função que criamos (essas strings) para saber como avaliar o nó de forma recursiva. Estamos criando um "array de structs".
	//OBSERVAÇÃO: Tomar cuidado para não criar p_id repetido, e de fazer o tratamento do p_id no parser (criar o símbolo certo ao ler
	// uma expressão) e na hora de printar a expressão  print_math_style
	...
	if (el->root->id<(NUM_FUNCTIONAL_SYMBOLS+NUM_AUXILIARY_FUNCTIONAL_SYMBOLS)){
		...
		switch (el->root->id) {
			...
			case 5:
                    ...
                    print_math_style (el->children[1], s);
            	break;
			//MODIFICAÇÃO: Essa função reconhece o ID do nó e decide como printar. Daqui para baixo são minhas expressões.
			//OBSERVAÇÃO: a aridade criada determina o número de filhos que o nó terá. Não tentar acessar children[1] se aridade for 1!!
			// Além disso, note que o código usa essa função para salvar os indivíduos em individuals.txt, então o jeito que imprimir 
			// cada símbolo aqui deve estar alinhado com o jeito que o parser vai ler e criar os nós! Tudo que for impresso aqui
			// deve ter uma forma de ser tratado no parser.
			case 6:
            		s = s + " _sqrt.abs ";
					print_math_style (el->children[0], s);
            	break;
...
void create_population(population **p, int i){
	...
	//OBSERVAÇÃO: expression_file permite que ele use informação de execuções passadas. Note que, em vários experimentos seguidos, 
	// é importante que isso seja 0, para evitar contaminar uma execução com informações de outras
...
double eval(node *tree){
	if(tree->root->type==1){
		...
		//MODIFICAÇÃO: Aqui devemos implementar a computação que um símbolo faz sobre seus filhos. Todos os símbolos
		// que forem criados devem ter uma forma de ser avaliados aqui. Desse ponto para baixo, são os símbolos que eu criei.
		//OBSERVAÇÃO: esse código é em C/C++: não vai dar NaN igual o python e seguir a vida, vai explodir se der erro.
		// se for feita uma operação que pode gerar um erro, fazer a implementação de forma segura.
...
//MODIFICAÇÃO: essa é a função de fitness aplicada sobre as expressões, PARA A PARTIÇÃO DE TREINO.
// Originalmente, era utilizado o MAE (apesar de o artigo dizer que é o RMSE...).
// Fiz a modificação para ser o RMSE.
double Myevaluate (node *el) {
	double d=0;
    vector <double> val;
...
//MODIFICAÇÃO: Aqui é feito o cálculo da função de fitness para o teste 
// (não sei pq duas funções seperadas).
// Modifiquei da mesma forma que a anterior;
double Myevaluate_test (node *el) {
	double d=0;
    vector <double> val;
...
//MODIFICAÇÃO: Aqui ele tbm calcula o fitness, originalmente utilizando o MAE. Modifiquei para
// calcular o RMSE.
void update_training_fitness(vector <double> semantic_values, bool crossover){
    double d=0;
    for(int j=0;j<nrow;j++){
...
//MODIFICAÇÃO: mesma coisa da função de cima.
void update_test_fitness(vector <double> semantic_values, bool crossover){
    double d=0;
...
void evaluate_unseen_new_data(population **p, ofstream& OUT){
    ...
    if(!in.is_open()) {
        ...
        char str[255];
		//OBSERVAÇÃO: nesse while o código faz a avaliação da melhor solução para a primeira geração. Como as demais gerações são construídas em cima dessa, 
		// então aqui é feito o suficiente para poder continuar a próxima geração e por fim avaliar como se fosse uma expressão
        while(true){ 
            in >> str;
            if(strcmp(str,"***")==0){
                break;
            ...
            if(index4==0){ 
				...
				//OBSERVAÇÃO: As expressões vem da combinação de uma sigmoide entre duas árvores
		...
		//OBSERVAÇÃO: agora que a base já foi avaliada, ele vai sair avaliando as demais gerações
       	while(!in.eof()){ 
            for(int i=0; i<config.population_size; i++){
	...
	//OBSERVAÇÃO: best é sempre o índice 5 da última geração.
    OUT<<eval_[best]<<endl; 
}