	...
	// if USE_TEST_SET is different from 1 the system will perform the usual evolutionary process
		...
		*/
		...
		for(int num_gen=0; num_gen<config.max_number_generations; num_gen++){	
			...
			//MODIFICAÇÃO: parar de imprimir a geração
			//cout<<"Generation "<<num_gen+1<<endl;

			// creation of a new population (without building trees!!)
			for(int k=0;k<config.population_size;k++){

				//PONTO DE INTERESSE: Esse trecho faz o sorteio de mutação ou crossover. Aqui ele segue os parâmetros 
				// da configuração, mas no artigo Geometric Semantic Genetic Programming for Real Life Applications
				// eles comentam sobre o uso aleatório desses valores de configuração.
				// Entretanto, na configuração o valor de mutation_step é definido mas nunca é utilizado, fica apenas
				// o aleatório
